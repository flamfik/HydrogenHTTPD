...
def request(path: str, body: bytes, boundary: str, token: str | None) -> bytes:
...
def main() -> int:
    if len(sys.argv) < 2:
        print("usage: integration_upload_auth_tests.py <hydrogen_httpd_exe>", file=sys.stderr)
        return 2
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        cfg.write_text("\n".join([
            ...
            f"upload_spool_directory = {spool}",
        ...
        try:
            ...
            assert_status(res, 200, "authorized upload")
            ...
            assert stored.read_bytes() == b"hello-stream-auth"
            assert not any(spool.iterdir()), "spool directory should be empty after successful upload"

            blocked = multipart("abc", "shell.php", b"<?php echo 1; ?>")
            assert_status(send_raw(18085, request("/__hydrogen/upload", blocked, "abc", "full-admin-secret")), 415, "blocked php upload")
            ...
            assert "token=\"adminonly\"" in audit_text

            print("integration upload auth tests passed")
            return 0
        finally:
            proc.terminate()
            try:
